...
export const FilterButtons: React.FC<FilterButtonsProps> = ({ selectedFilter, onFilterChange, styleHelpers }) => {
    const { getButtonBgClass } = styleHelpers;
    ...
    return (
        <div className="mb-4 flex space-x-2 overflow-x-auto pb-2">
            {filters.map((filter) => (
                <button
                    key={filter.id}
                    className={`${getButtonBgClass(selectedFilter === filter.id)} flex items-center space-x-2 rounded-full px-4 py-2 text-sm font-medium whitespace-nowrap transition-all`}

...
export const BottomNavigation: React.FC<BottomNavigationProps> = ({ styleHelpers }) => {
    ...
    return (
        <div className={`fixed inset-x-0 bottom-0 z-10 border-t ${getCardBgClass()} p-3 shadow-lg`}>
            ...
            {/*            {item.isCenter ? (*/}
            {/*                <div*/}
            {/*                    className="mx-auto -mt-5 flex h-12 w-12 items-center justify-center rounded-full shadow-lg"*/}
            ...
            {/*            ) : (*/}
            {/*                <>*/}
            {/*                    <i className={`fas ${item.icon} text-lg`} style={item.isActive ? { color: customColor } : { color: '#9CA3AF' }}></i>*/}
